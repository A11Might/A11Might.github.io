...
面对一个深度优先搜索问题，最重要的就是如何才可以搜得不重不漏。比如说走迷宫和图的遍历，只要按照某种顺序做尝试，比如网格图中的上右下左的顺序，那么就可以保证不重不漏地搜遍整个迷宫或者图，枚举出所有可能的情况。而对于排列与组合问题，我们往往需要记录当前已经取走哪些元素，考虑下一步按照什么样的顺序继续取用哪些元素。
