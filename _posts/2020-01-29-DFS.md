...
DFS 是按照一条路径一条路径搜索的，其本质就是枚举，枚举所有可能的情况，从而找到一个（可达性）或者若干个可行解（回溯）。当题目要求你遍历全部或者大部分情况，直到找到合法情况或者所有答案的问题，我们就可以使用 DFS 解决问题。比如：遍历给定图找到可行路径、全排列方案、组合方案……或者过于复杂只能通过枚举所有情况求解的问题。
