...
DFS 的本质就是枚举，通过枚举状态空间中的所有所有状态，从而找到一个（可达性）或者若干个可行解（回溯）。当题目要求你遍历全部或者大部分情况，直到找到合法情况或者所有答案的问题，我们就可以使用 DFS 解决问题。比如：遍历给定图找到可行路径、全排列方案、组合方案……或者过于复杂只能通过枚举所有情况求解的问题。

面对一个深度优先搜索问题，最重要的就是确定状态空间，也就是我们要搜什么，怎么搜，如何才可以搜得不重不漏。比如说走迷宫和图的遍历，那么状态就是当前在哪一个点，只要按照某种顺序做尝试，比如网格图中的上右下左的顺序，那么就可以保证不重不漏地搜遍整个状态空间。而对于排列与组合问题，我们往往需要记录当前已经取走哪些元素，考虑下一步按照什么样的顺序继续取用哪些元素。

此外，我们可以根据实际情况，在状态空间树中剪去一些根本没有可能到达答案的分支（剪枝），从而减少需要搜索空间大小达到加速的效果。
...
- 使用一个 visited 数组标记一个节点是否已经访问过了，如果访问过了就不再访问，这样就可以不重不漏的遍历完一张图中的所有节点。
...
{: .box-note}
**Note：** 上述解法是标准的 DFS 模板，利用递归栈，在得到新的节点时立即对该新节点进行遍历。若想使用栈的迭代解法，可以联想树的 DFS，同理。 
