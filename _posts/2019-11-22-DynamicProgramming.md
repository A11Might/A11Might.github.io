...
而最长路径问题则不具有最优子结构属性, 这里的最长路径是指两个节点之间的最长简单路径(无循环的路径). 如下, 从 q 到 t 有两天最长路径: q -> r -> t 和 q -> s -> t. 而最长路径 q -> r -> t 不是从 q 到 r 的最长路径和从 r 到 t 的最长路径的组合(q 到 r 的最长路径是 q -> s -> t -> r; r 到 t 的最长路径是 r -> q -> s -> t).
