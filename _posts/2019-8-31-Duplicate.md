...
所以我们将区间[1, n - 1]从中间数字m处，拆分成两部分[1, m]和[m + 1, n - 1]，分别将nums数组中的数字放入两区间中，若[1, m]区间内包含nums数组中的数字的数目超过m，则重复数字一定在[1, m]范围内；否则重复数字就一定在[m + 1, n - 1]范围内，然后继续把一定含有重复数字的区间一分为二，直至找到一个重复数字
